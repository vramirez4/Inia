...
VEPPULL = function(csv) {
  for (i in seq(1, dim(csv)[1])) {
    vepcommand = sprintf(
      "vep --buffer_size 10000 --cache --check_existing --format vcf --gene_phenotype --input_file %s.vcf --offline --output_file %s.tsv --phased --polyphen b --pubmed --regulatory --sift b --tab --total_length --variant_class --number --force_overwrite
      ",
      csv[i, 1],
      csv[i, 1]
    )
